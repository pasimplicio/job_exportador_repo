--202203: Deve ser substituida pela referencia do faturamento que se deseja obter os dados
...
SELECT 
	...
	(
		...
		WHERE
			ics.imov_id = imo.imov_id AND ics.cbst_id IN (12,14,17)
	...
	con_ult_fat.cagua AS "VOL AG 202203",
	TO_CHAR(con_ult_fat.vl_agua, '999G999G990D00') AS "VL AG 202203",
	con_ult_fat.cesg AS "VOL ES 202203",
	TO_CHAR(con_ult_fat.vl_esgoto, '999G999G990D00') AS "VL ES 202203",
	TO_CHAR(con_ult_fat.vl_debitos, '999G999G990D00') AS "OUTROS SERVICOS 202203",
	TO_CHAR(con_ult_fat.vl_creditos, '999G999G990D00') AS "CRED 202203",
	TO_CHAR(con_ult_fat.vl_impostos, '999G999G990D00') AS "IMPOSTOS 202203",
	TO_CHAR(con_ult_fat.valor, '999G999G990D00') AS "VALOR 202203",
	con_pen_fat.cagua AS "VOL AG 202203 - 1",
	TO_CHAR(con_pen_fat.vl_agua, '999G999G990D00') AS "VL AG 202203 - 1",
	con_pen_fat.cesg AS "VOL ES 202203 - 1",
	TO_CHAR(con_pen_fat.vl_esgoto, '999G999G990D00') AS "VL ES 202203 - 1",
	TO_CHAR(con_pen_fat.vl_debitos, '999G999G990D00') AS "OUTROS SERVICOS 202203 - 1",
	TO_CHAR(con_pen_fat.vl_creditos, '999G999G990D00') AS "CRED 202203 - 1",
	TO_CHAR(con_pen_fat.vl_impostos, '999G999G990D00') AS "IMPOSTOS 202203 - 1",
	TO_CHAR(con_pen_fat.valor, '999G999G990D00') AS "VALOR 202203 - 1",
	con_ant_fat.cagua AS "VOL AG 202203 - 2",
	TO_CHAR(con_ant_fat.vl_agua, '999G999G990D00') AS "VL AG 202203 - 2",
	con_ant_fat.cesg AS "VOL ES 202203 - 2",
	TO_CHAR(con_ant_fat.vl_esgoto, '999G999G990D00') AS "VL ES 202203 - 2",
	TO_CHAR(con_ant_fat.vl_debitos, '999G999G990D00') AS "OUTROS SERVICOS 202203 - 2",
	TO_CHAR(con_ant_fat.vl_creditos, '999G999G990D00') AS "CRED 202203 - 2",
	TO_CHAR(con_ant_fat.vl_impostos, '999G999G990D00') AS "IMPOSTOS 202203 - 2",
	TO_CHAR(con_ant_fat.valor, '999G999G990D00') AS "VALOR 202203 - 2",
	...
	TO_CHAR(con_a_prescrever.valor, '999G999G990D00') AS "VALOR TOTAL MAIOR QUE 8 ANOS",
	con_a_prescrever.qtd AS "QTD. CONTAS MAIOR QUE 8 ANOS",
	con_a_prescrever.min AS "MENOR REFERENCIA MAIOR QUE 8 ANOS",
	con_a_prescrever.max AS "MAIOR REFERENCIA MAIOR QUE 8 ANOS",
	con_a_prescrever.min_venc AS "MENOR VENCIMENTO MAIOR QUE 8 ANOS",
	con_a_prescrever.max_venc AS "MAIOR VENCIMENTO MAIOR QUE 8 ANOS",
	TO_CHAR(con_a_prescrever.multa, '999G999G990D00') AS "MULTAS MAIOR QUE 8 ANOS",
	TO_CHAR(con_a_prescrever.juros, '999G999G990D00') AS "JUROS MAIOR QUE 8 ANOS",
	TO_CHAR((con_a_prescrever.multa+con_a_prescrever.juros+con_a_prescrever.valor), '999G999G990D00') AS "VALOR TOTAL COM JUROS MAIOR QUE 8 ANOS"
FROM 
	cadastro.imovel imo
	...
	LEFT JOIN micromedicao.consumo_historico cosh_a1 ON cosh_a1.imov_id = imo.imov_id AND cosh_a1.cshi_amfaturamento = 202203 AND cosh_a1.lgti_id = 1
	...
	LEFT JOIN micromedicao.medicao_historico mdh ON mdh.hidi_id = his.hidi_id AND mdh.mdhi_amleitura = 202203
	...
	LEFT JOIN (	SELECT 
			...
			FROM faturamento.conta con4 
			WHERE 
				con4.dcst_idatual IN (0,1,2) AND NOT EXISTS ( SELECT pag.cnta_id FROM arrecadacao.pagamento pag WHERE pag.cnta_id = con4.cnta_id) AND con4.cnta_dtvencimentoconta < CURRENT_DATE AND con4.cnta_dtrevisao IS NULL AND con4.iper_id <> 6 AND con4.cnta_amreferenciaconta <= (202203 - 800)
			GROUP BY 1) AS con_a_prescrever ON con_a_prescrever.mat1 = imo.imov_id
	LEFT JOIN (	SELECT 
				con4.imov_id AS mat1,
	...
	LEFT JOIN (	SELECT 
			...
			WHERE
				con4.cnta_amreferenciaconta = 202203
			UNION
			SELECT 
				con4.imov_id AS mat1,
			...
			FROM faturamento.conta_historico con4 
				INNER JOIN faturamento.conta_impressao cni ON cni.cnta_id = con4.cnta_id
			WHERE
				con4.cnhi_amreferenciaconta = 202203
			UNION
			SELECT 
				con4.imov_id AS mat1,
				con4.cnta_nnconsumoagua AS cagua,
	...
	LEFT JOIN (	SELECT 
			...
			WHERE
				con4.cnta_amreferenciaconta = (
					SELECT 
						CASE (CAST(SUBSTRING(CAST(202203 AS TEXT) FROM 5 FOR 6) AS INT))
							WHEN 1 THEN CAST((CAST(SUBSTRING(CAST(202203 AS TEXT) FROM 0 FOR 5) AS INT))-1 AS TEXT)||'12'
							ELSE CAST(202203-1 AS TEXT)
						END
				)
			UNION
			SELECT 
		...
		LEFT JOIN (	SELECT 
			...
			WHERE
				con4.cnta_amreferenciaconta = (
					SELECT 
						CASE (CAST(SUBSTRING(CAST(202203 AS TEXT) FROM 5 FOR 6) AS INT))
							WHEN 1 THEN CAST((CAST(SUBSTRING(CAST(202203 AS TEXT) FROM 0 FOR 5) AS INT))-1 AS TEXT)||'11'
							WHEN 2 THEN CAST((CAST(SUBSTRING(CAST(202203 AS TEXT) FROM 0 FOR 5) AS INT))-1 AS TEXT)||'12'
							ELSE CAST(202203-2 AS TEXT)
						END
				)
			UNION
			SELECT 
...
WHERE
	...
	--AND con_a_prescrever.qtd > 0 
	AND imo.iper_id IN (1,2)
