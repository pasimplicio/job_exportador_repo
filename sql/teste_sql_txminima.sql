WITH fat AS (
    ...
    FROM (
        ...
        WHERE con.dcst_idatual IN (0,1,2)
               AND con.imov_id IN (2648121,1749196)  
...
FROM (
    SELECT 
        con.imov_id AS mat1,
        1 AS qtd,
        COALESCE(catc.ctcg_qteconomia, 1) AS economias,
        (
            ...
            /* ===== MULTIPLICADOR DE ECONOMIAS (SOMENTE RESIDENCIAL) ===== */
            * CASE 
...
SELECT
    ...
    -- passivo taxa minima
    con_fat_a2.economias AS "Qtde economias tx minima",
    con_fat_a2.qtd AS "Qtde faturas",
...
WHERE
    ...
    AND imo.imov_id IN (2648121,1749196)  
