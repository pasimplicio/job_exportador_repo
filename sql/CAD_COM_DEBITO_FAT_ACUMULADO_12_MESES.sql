...
FROM 
	...
	LEFT JOIN (	
			SELECT
				fat_med.mat1 AS mat1,
				SUM(fat_med.qtd) AS qtd,
				SUM(fat_med.cagua) AS cagua,
				SUM(fat_med.vl_agua) AS vl_agua,
				SUM(fat_med.cesg) AS cesg,
				SUM(fat_med.vl_esgoto) AS vl_esgoto,
				SUM(fat_med.vl_debitos) AS vl_debitos,
				SUM(fat_med.vl_creditos) AS vl_creditos,
				SUM(fat_med.vl_impostos) AS vl_impostos,
				SUM(fat_med.valor) AS valor
			FROM
				(SELECT 
					con4.imov_id AS mat1,
					COUNT(con4.cnta_id) AS qtd,
