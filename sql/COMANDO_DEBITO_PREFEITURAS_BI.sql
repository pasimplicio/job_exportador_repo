...
FROM 
	...
	LEFT JOIN (	SELECT 
			...
			WHERE 
				con4.dcst_idatual IN (0,1,2) AND NOT EXISTS ( SELECT pag.cnta_id FROM arrecadacao.pagamento pag WHERE pag.cnta_id = con4.cnta_id) AND con4.cnta_dtvencimentoconta <= CURRENT_DATE AND con4.cnta_amreferenciaconta > 201407 AND con4.iper_id = 6
	...
	LEFT JOIN (	SELECT 
			...
			FROM faturamento.conta con4 
				INNER JOIN cadastro.localidade loc4 ON loc4.loca_id = con4.loca_id AND loc4.greg_id IN (1,2)
			WHERE 
				con4.dcst_idatual IN (0,1,2) AND NOT EXISTS ( SELECT pag.cnta_id FROM arrecadacao.pagamento pag WHERE pag.cnta_id = con4.cnta_id) AND con4.cnta_dtvencimentoconta <= CURRENT_DATE AND con4.cnta_amreferenciaconta > 201407 AND NOT con4.cnta_dtrevisao IS NULL
	...
	LEFT JOIN (	SELECT
			...
			FROM faturamento.conta con4 
				INNER JOIN cadastro.localidade loc4 ON loc4.loca_id = con4.loca_id AND loc4.greg_id IN (1,2)
			WHERE 
				con4.dcst_idatual IN (0,1,2) AND NOT EXISTS ( SELECT pag.cnta_id FROM arrecadacao.pagamento pag WHERE pag.cnta_id = con4.cnta_id) AND con4.cnta_dtvencimentoconta <= CURRENT_DATE AND con4.cnta_amreferenciaconta > 201407 AND con4.cnta_dtrevisao IS NULL AND con4.iper_id <> 6
