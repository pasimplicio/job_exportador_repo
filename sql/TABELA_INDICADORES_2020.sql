SELECT * FROM(
	SELECT
		...
		FROM
			...
			CROSS JOIN (SELECT  referencia AS "REFERENCIA" FROM generate_series(202001, 202012) referencia) AS refs
			LEFT JOIN
				(
					SELECT 		
				...
				FROM
					...
					WHERE
						con.cnta_amreferenciaconta>=((TO_CHAR('2020-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND con.cnta_amreferenciaconta<=(TO_CHAR('2020-12-31'::DATE, 'YYYYMM')::INTEGER) 
					GROUP BY 1,2
				UNION
					SELECT
						une.uneg_id AS uneg_id,
					...
					WHERE
						con.cnhi_amreferenciaconta>=((TO_CHAR('2020-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND con.cnhi_amreferenciaconta<=(TO_CHAR('2020-12-31'::DATE, 'YYYYMM')::INTEGER) 
					GROUP BY 1,2
				UNION
					SELECT
							une.uneg_id AS uneg_id,
					...
					FROM(
						SELECT
							...
							FROM
								...
								INNER JOIN cadastro.unidade_negocio une ON une.uneg_id = loc.uneg_id
							WHERE
								con.cnta_amreferenciaconta>=201901 AND con.cnta_amreferenciaconta<= ('2019'||TO_CHAR('2020-12-31'::DATE, 'MM'))::INTEGER
							GROUP BY 1,2
						UNION
							SELECT
								une.uneg_id AS uneg_id,
							...
							FROM
								...
								INNER JOIN cadastro.unidade_negocio une ON une.uneg_id = loc.uneg_id
							WHERE
								con.cnhi_amreferenciaconta>=201901 AND con.cnhi_amreferenciaconta<= ('2019'||TO_CHAR('2020-12-31'::DATE, 'MM'))::INTEGER
							GROUP BY 1,2
						UNION
							SELECT
								une.uneg_id AS uneg_id,
				...
				(
					...
					(	SELECT
						...
						WHERE
							...
							TO_CHAR(con.cnta_dtretificacao, 'YYYYMM')::INTEGER>=((TO_CHAR('2020-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND TO_CHAR(con.cnta_dtretificacao, 'YYYYMM')::INTEGER<=(TO_CHAR('2020-12-31'::DATE, 'YYYYMM')::INTEGER)
						GROUP BY 1,2
					UNION
						SELECT
							loc.uneg_id AS uneg_id,
						...
						WHERE
							con.dcst_idatual IN (1,5) AND
							TO_CHAR(con.cnhi_dtretificacao, 'YYYYMM')::INTEGER>=((TO_CHAR('2020-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND TO_CHAR(con.cnhi_dtretificacao, 'YYYYMM')::INTEGER<=(TO_CHAR('2020-12-31'::DATE, 'YYYYMM')::INTEGER)
						GROUP BY 1,2
					UNION
						SELECT
							loc.uneg_id AS uneg_id,
					...
					UNION
						...
						WHERE
							...
							TO_CHAR(con.cnta_dtcancelamento, 'YYYYMM')::INTEGER>=((TO_CHAR('2020-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND TO_CHAR(con.cnta_dtcancelamento, 'YYYYMM')::INTEGER<=(TO_CHAR('2020-12-31'::DATE, 'YYYYMM')::INTEGER)
						GROUP BY 1,2
					UNION
						SELECT
							loc.uneg_id AS uneg_id,
						...
						WHERE
							con.dcst_idatual = 3 AND
							TO_CHAR(con.cnhi_dtcancelamento, 'YYYYMM')::INTEGER>=((TO_CHAR('2020-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND TO_CHAR(con.cnhi_dtcancelamento, 'YYYYMM')::INTEGER<=(TO_CHAR('2020-12-31'::DATE, 'YYYYMM')::INTEGER)
						GROUP BY 1,2) AS ind
				...
				FROM
					(
						...
						WHERE
							...
							--AND tbc.tbca_tmultimaalteracao >= ('2020-12-31'::DATE::TIMESTAMP)
							AND TO_CHAR(tbc.tbca_tmultimaalteracao,'YYYYMM')::INTEGER>=((TO_CHAR('2020-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND TO_CHAR(tbc.tbca_tmultimaalteracao,'YYYYMM')::INTEGER<=(TO_CHAR('2020-12-31'::DATE, 'YYYYMM')::INTEGER)
			...
			LEFT JOIN
				(
					...
					FROM(
						...
						WHERE
							res.ardd_amreferenciaarrecadacao>=201901 AND res.ardd_amreferenciaarrecadacao<= ('2019'||TO_CHAR('2020-12-31'::DATE, 'MM'))::INTEGER
						GROUP BY 1,2) AS ind
				...
				(
					...
					FROM(
						...
						FROM
							...
							LEFT JOIN cadastro.localidade loc ON loc.loca_id = res.loca_id
						WHERE
							res.ardd_amreferenciaarrecadacao >= ((TO_CHAR('2020-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND res.ardd_amreferenciaarrecadacao <= (TO_CHAR('2020-12-31'::DATE, 'YYYYMM')::INTEGER)
						GROUP BY 1,2) AS ind
...
UNION
	SELECT
		...
		FROM
			cadastro.unidade_negocio une
			CROSS JOIN (SELECT  referencia AS "REFERENCIA" FROM generate_series(202101, 202112) referencia) AS refs
			LEFT JOIN
				(
					SELECT 		
					ind.uneg_id AS uneg_id,
				...
				FROM
					...
					FROM
						...
						LEFT JOIN micromedicao.hidrometro_inst_hist his ON con.imov_id = his.lagu_id AND hidi_dtinstalacaohidrometro <= con.cnta_dtemissao AND (his.hidi_dtretiradahidrometro IS NULL OR his.hidi_dtretiradahidrometro>con.cnta_dtemissao)
					WHERE
						con.cnta_amreferenciaconta>=((TO_CHAR('2021-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND con.cnta_amreferenciaconta<=(TO_CHAR('2021-12-31'::DATE, 'YYYYMM')::INTEGER) 
					GROUP BY 1,2
				UNION
					SELECT
						une.uneg_id AS uneg_id,
					...
					FROM
						...
						LEFT JOIN micromedicao.hidrometro_inst_hist his ON con.imov_id = his.lagu_id AND hidi_dtinstalacaohidrometro <= con.cnhi_dtemissao AND (his.hidi_dtretiradahidrometro IS NULL OR his.hidi_dtretiradahidrometro>con.cnhi_dtemissao)
					WHERE
						con.cnhi_amreferenciaconta>=((TO_CHAR('2021-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND con.cnhi_amreferenciaconta<=(TO_CHAR('2021-12-31'::DATE, 'YYYYMM')::INTEGER) 
					GROUP BY 1,2
				UNION
					SELECT
							une.uneg_id AS uneg_id,
					...
					FROM(
						SELECT
							...
							FROM
								...
								INNER JOIN cadastro.unidade_negocio une ON une.uneg_id = loc.uneg_id
							WHERE
								con.cnta_amreferenciaconta>=201901 AND con.cnta_amreferenciaconta<= ('2019'||TO_CHAR('2021-12-31'::DATE, 'MM'))::INTEGER
							GROUP BY 1,2
						UNION
							SELECT
								une.uneg_id AS uneg_id,
							...
							FROM
								...
								INNER JOIN cadastro.unidade_negocio une ON une.uneg_id = loc.uneg_id
							WHERE
								con.cnhi_amreferenciaconta>=201901 AND con.cnhi_amreferenciaconta<= ('2019'||TO_CHAR('2021-12-31'::DATE, 'MM'))::INTEGER
							GROUP BY 1,2
						UNION
							SELECT
								une.uneg_id AS uneg_id,
						...
						UNION
							...
							WHERE
								con.cnhi_amreferenciaconta>=201901 AND con.cnhi_amreferenciaconta<= ('2019'||TO_CHAR('2021-12-31'::DATE, 'MM'))::INTEGER
							GROUP BY 1,2) AS ind
						GROUP BY 1,2) AS faturamento_acm_2019 ON faturamento_acm_2019.uneg_id = une.uneg_id AND faturamento_acm_2019."REFERENCIA" = refs."REFERENCIA"-200
			LEFT JOIN
				(
					SELECT
						ind.uneg_id AS uneg_id,
			...
			LEFT JOIN
				(
					...
					(	SELECT
						...
						WHERE
							con.dcst_idatual IN (1,5) AND
							TO_CHAR(con.cnta_dtretificacao, 'YYYYMM')::INTEGER>=((TO_CHAR('2021-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND TO_CHAR(con.cnta_dtretificacao, 'YYYYMM')::INTEGER<=(TO_CHAR('2021-12-31'::DATE, 'YYYYMM')::INTEGER)
						GROUP BY 1,2
					UNION
						SELECT
							loc.uneg_id AS uneg_id,
						...
						WHERE
							con.dcst_idatual IN (1,5) AND
							TO_CHAR(con.cnhi_dtretificacao, 'YYYYMM')::INTEGER>=((TO_CHAR('2021-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND TO_CHAR(con.cnhi_dtretificacao, 'YYYYMM')::INTEGER<=(TO_CHAR('2021-12-31'::DATE, 'YYYYMM')::INTEGER)
						GROUP BY 1,2
					UNION
						SELECT
							loc.uneg_id AS uneg_id,
					...
					UNION
						...
						WHERE
							con.dcst_idatual = 3 AND
							TO_CHAR(con.cnta_dtcancelamento, 'YYYYMM')::INTEGER>=((TO_CHAR('2021-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND TO_CHAR(con.cnta_dtcancelamento, 'YYYYMM')::INTEGER<=(TO_CHAR('2021-12-31'::DATE, 'YYYYMM')::INTEGER)
						GROUP BY 1,2
					UNION
						SELECT
							loc.uneg_id AS uneg_id,
						...
						WHERE
							con.dcst_idatual = 3 AND
							TO_CHAR(con.cnhi_dtcancelamento, 'YYYYMM')::INTEGER>=((TO_CHAR('2021-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND TO_CHAR(con.cnhi_dtcancelamento, 'YYYYMM')::INTEGER<=(TO_CHAR('2021-12-31'::DATE, 'YYYYMM')::INTEGER)
						GROUP BY 1,2) AS ind
					GROUP BY 1,2) AS ind_refat ON ind_refat.uneg_id = une.uneg_id AND ind_refat."REFERENCIA" = refs."REFERENCIA"
			LEFT JOIN
			(
				...
				FROM
					(
						...
						WHERE
							...
							--AND 
							tbc.tbco_id IN (271, 272, 275, 276, 701, 60, 4796, 23848, 23850, 1527, 1529, 1531, 1539, 2524, 2525, 2526, 2527, 2529)
							--AND tbc.tbca_tmultimaalteracao >= ('2021-12-31'::DATE::TIMESTAMP)
							AND TO_CHAR(tbc.tbca_tmultimaalteracao,'YYYYMM')::INTEGER>=((TO_CHAR('2021-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND TO_CHAR(tbc.tbca_tmultimaalteracao,'YYYYMM')::INTEGER<=(TO_CHAR('2021-12-31'::DATE, 'YYYYMM')::INTEGER)
							--AND usu.usur_nmlogin = 'YGOR'
						GROUP BY 1,2
						ORDER BY 2 DESC) AS ind
			) AS ind_cadastro ON ind_cadastro.uneg_id = une.uneg_id AND ind_cadastro."REFERENCIA" = refs."REFERENCIA"
				...
				(
					...
					FROM(
						...
						FROM
							...
							LEFT JOIN cadastro.localidade loc ON loc.loca_id = res.loca_id
						WHERE
							res.ardd_amreferenciaarrecadacao>=201901 AND res.ardd_amreferenciaarrecadacao<= ('2019'||TO_CHAR('2021-12-31'::DATE, 'MM'))::INTEGER
						GROUP BY 1,2) AS ind
						GROUP BY 1,2) AS arrecadacao_acm_2019 ON arrecadacao_acm_2019.uneg_id = une.uneg_id AND arrecadacao_acm_2019."REFERENCIA" = refs."REFERENCIA"-200
			LEFT JOIN
				(
					SELECT
						ind.uneg_id AS uneg_id,
					...
					FROM(
						...
						FROM
							...
							LEFT JOIN cadastro.localidade loc ON loc.loca_id = res.loca_id
						WHERE
							res.ardd_amreferenciaarrecadacao >= ((TO_CHAR('2021-12-31'::DATE, 'YYYY')||'01')::INTEGER) AND res.ardd_amreferenciaarrecadacao <= (TO_CHAR('2021-12-31'::DATE, 'YYYYMM')::INTEGER)
						GROUP BY 1,2) AS ind
						GROUP BY 1,2) AS arrecadacao_acm_2020 ON arrecadacao_acm_2020.uneg_id = une.uneg_id AND arrecadacao_acm_2020."REFERENCIA" = refs."REFERENCIA"
	) AS dados
ORDER BY 2,1