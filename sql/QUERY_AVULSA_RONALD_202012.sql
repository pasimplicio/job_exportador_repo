	...
	WHERE
		con.cnhi_dtvencimentooriginal >= '2020-12-01' AND con.cnhi_dtvencimentooriginal <= '2020-12-31'
UNION
	SELECT
		imo.imov_id AS "MATRICULA",
		cli.clie_nmcliente AS "NOME",
...
UNION
	...
	FROM
		...
		LEFT JOIN faturamento.faturamento_grupo ftg ON rot.ftgr_id = ftg.ftgr_id
	WHERE
		con.cnta_dtvencimentooriginal >= '2020-12-01' AND con.cnta_dtvencimentooriginal <= '2020-12-31'
UNION
	SELECT
		imo.imov_id AS "MATRICULA",
		cli.clie_nmcliente AS "NOME",
...
UNION
	...
	FROM
		...
		LEFT JOIN faturamento.faturamento_grupo ftg ON rot.ftgr_id = ftg.ftgr_id
	WHERE
		con.cnta_dtvencimentooriginal >= '2020-12-01' AND 
		...
		AND con.cnta_dtvencimentooriginal <= '2020-12-31'
	UNION
	SELECT
		imo.imov_id AS "MATRICULA",
		cli.clie_nmcliente AS "NOME",
...
UNION
	SELECT
		...
		ftg.ftgr_dsfaturamentogrupo AS "GRUPO FATURAMENTO",
		dis.diop_dsdistritooperacional AS "DISTR OPERACIONAL",
		CURRENT_DATE - con.cnhi_dtvencimentoconta AS "ATRASO",
		con.cnhi_amreferenciaconta AS "REFERENCIA",
		NULL AS "DATA PAGAMENTO",
		con.cnhi_dtvencimentoconta AS "VENCIMENTO",
		con.cnhi_dtvencimentooriginal AS "VENCIMENTO ORIGINAL",
		CURRENT_DATE - con.cnhi_dtvencimentooriginal AS "ATRASO VENCIMENTO ORIGINAL",
		(CASE 
		WHEN CURRENT_DATE - con.cnhi_dtvencimentooriginal <= 0 THEN '01 - EM DIA'
		WHEN CURRENT_DATE - con.cnhi_dtvencimentooriginal <= 5  THEN '02 - ATE 5 DIAS DE ATRASO'
		WHEN CURRENT_DATE - con.cnhi_dtvencimentooriginal <= 15  THEN '03 - ATE 15 DIAS DE ATRASO'
		WHEN CURRENT_DATE - con.cnhi_dtvencimentooriginal <= 30  THEN '04 - ATE 30 DIAS DE ATRASO'
		WHEN CURRENT_DATE - con.cnhi_dtvencimentooriginal <= 60  THEN '05 - ATE 60 DIAS DE ATRASO'
		WHEN CURRENT_DATE - con.cnhi_dtvencimentooriginal <= 90  THEN '06 - ATE 90 DIAS DE ATRASO'
		WHEN CURRENT_DATE - con.cnhi_dtvencimentooriginal <= 180  THEN '07 - ATE 180 DIAS DE ATRASO'
		WHEN CURRENT_DATE - con.cnhi_dtvencimentooriginal <= 365  THEN '08 - ATE 365 DIAS DE ATRASO'
		WHEN CURRENT_DATE - con.cnhi_dtvencimentooriginal <= 1825  THEN '09 - ATE 5 ANOS DE ATRASO'
		WHEN CURRENT_DATE - con.cnhi_dtvencimentooriginal <= 3650  THEN '10 - ATE 10 ANOS DE ATRASO'
		WHEN CURRENT_DATE - con.cnhi_dtvencimentooriginal > 3650  THEN '11 - ACIMA DE 10 ANOS DE ATRASO'
		END) AS "FAIXA ATRASO",
		TO_CHAR((con.cnhi_vlagua),'L999G999G990D00') AS "VL AGUA",
		TO_CHAR((con.cnhi_vlesgoto),'L999G999G990D00') AS "VL ESGOTO",
		TO_CHAR((con.cnhi_vldebitos),'L999G999G990D00') AS "VL DEBITOS",
	...
	FROM
		...
		LEFT JOIN faturamento.faturamento_grupo ftg ON rot.ftgr_id = ftg.ftgr_id
	WHERE
		con.cnhi_dtvencimentooriginal >= '2020-12-01' AND con.cnhi_dtvencimentooriginal <= '2020-12-31' AND 
		con.dcst_idatual = 5