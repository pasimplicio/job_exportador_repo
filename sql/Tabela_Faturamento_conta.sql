--202401: Deve ser substituida pela referencia do faturamento que se deseja obter os dados
...
SELECT 
	...
	TO_CHAR(SUM(con_ult_fat.valor), '999G999G990D00') AS "Valor Faturado"
FROM 
	cadastro.imovel imo
	...
	LEFT JOIN micromedicao.consumo_historico cosh_a1 ON cosh_a1.imov_id = imo.imov_id AND cosh_a1.cshi_amfaturamento = 202401 AND cosh_a1.lgti_id = 1
	...
	LEFT JOIN micromedicao.medicao_historico mdh ON mdh.hidi_id = his.hidi_id AND mdh.mdhi_amleitura = 202401
	...
	LEFT JOIN (	SELECT 
			...
			WHERE
				con4.cnta_amreferenciaconta = 202401
			UNION
			SELECT 
				con4.imov_id AS mat1,
			...
			FROM faturamento.conta_historico con4 
				INNER JOIN faturamento.conta_impressao cni ON cni.cnta_id = con4.cnta_id
			WHERE
				con4.cnhi_amreferenciaconta = 202401
			UNION
			SELECT 
				con4.imov_id AS mat1,
				con4.cnta_nnconsumoagua AS cagua,
	...
	LEFT JOIN (	SELECT 
			...
			WHERE
				con4.cnta_amreferenciaconta = (
					SELECT 
						CASE (CAST(SUBSTRING(CAST(202401 AS TEXT) FROM 5 FOR 6) AS INT))
							WHEN 1 THEN CAST((CAST(SUBSTRING(CAST(202401 AS TEXT) FROM 0 FOR 5) AS INT))-1 AS TEXT)||'12'
							ELSE CAST(202401-1 AS TEXT)
						END
				)
			UNION
			SELECT 
		...
		LEFT JOIN (	SELECT 
			...
			WHERE
				con4.cnta_amreferenciaconta = (
					SELECT 
						CASE (CAST(SUBSTRING(CAST(202401 AS TEXT) FROM 5 FOR 6) AS INT))
							WHEN 1 THEN CAST((CAST(SUBSTRING(CAST(202401 AS TEXT) FROM 0 FOR 5) AS INT))-1 AS TEXT)||'11'
							WHEN 2 THEN CAST((CAST(SUBSTRING(CAST(202401 AS TEXT) FROM 0 FOR 5) AS INT))-1 AS TEXT)||'12'
							ELSE CAST(202401-2 AS TEXT)
						END
				)
			UNION
			SELECT 
			...
			FROM		
				(SELECT 
					...
					WHERE
						...
						AND pag.pgmt_amreferenciaarrecadacao = 202401
					GROUP BY 1
				UNION
					SELECT 
						pag.imov_id AS imov_id,
					...
					WHERE
						pag.pgst_idatual IN (0,1)
						AND pag.pghi_amreferenciaarrecadacao = 202401
					GROUP BY 1) pags
			GROUP BY 1
...
LEFT JOIN (
			...
			WHERE
				con.cnhi_amreferenciaconta = 202401
		UNION
			SELECT
				con.imov_id AS "MATRICULA",
		...
		UNION
			...
			WHERE
				con.cnta_amreferenciaconta = 202401
		UNION
			SELECT
				con.imov_id AS "MATRICULA",
		...
		UNION
			...
			WHERE
				con.cnta_amreferenciaconta = 202401 AND 
		...
		UNION
			...
			WHERE
				con.cnhi_amreferenciaconta = 202401 AND 
				con.dcst_idatual = 5
...
WHERE
	...
	--AND imo.imov_id = 896225
	AND con_ult_fat.valor > 0
--ORDER BY "LOCALIDADE"