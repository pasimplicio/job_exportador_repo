SELECT
	...
	tbl.tbla_id1 AS "CLIENTE ID",
	TO_CHAR(tbc.tbca_tmultimaalteracao, 'dd/MM/yyyy') AS "DATA ALTERACAO",
	tco.tbco_dscoluna AS "DADO ALTERADO",
	tco.tbco_id,
	tbc.tbca_cncolunaanterior AS "VALOR ANTERIOR",
	tbc.tbca_cncolunaatual AS "VALOR ATUAL",
	usu.usur_nmusuario AS "USUARIO ALTERACAO"
FROM 
	cadastro.imovel imo
	...
	INNER JOIN seguranca.tabela_linha_alteracao tbl ON /*tbl.tabe_id = 60 AND */ tbL.tbla_id1 = cli.clie_id
	...
	INNER JOIN seguranca.usuario_alteracao usa ON usa.tref_id = tbl.tref_id
	INNER JOIN seguranca.usuario usu ON usu.usur_id = usa.usis_id
...
WHERE 
	...
	--AND tbc.tbco_id IN (271, 272, 275, 276, 701, 60)
	AND tbc.tbco_id IN (271,272,275,276,701,4796)
	AND tbc.tbca_tmultimaalteracao >= '31/07/2021'

	--AND usu.usur_nmusuario IN ('GUILHERME FERREIRA COSTA', 'JANETE OLIVEIRA SILVA', 'LANA BARROS E SILVA', 
	--'LUIS MAGNO DOS SANTOS PEREIRA', 'ZULEIDE DA CONCEICAO VIEGAS FONSECA', 'IVONEIDE MARIA CELESTE ALVES', 'MARCO AURELIO REIS LOUREIRO', 
	--'MARIA DA CONCEICAO MARTINS SILVA', 'THIAGO CHRISTOPHER CARDOSO ARAUJO', 'MURILO BRENNER', 'KATIANA ALMEIDA ARAUJO DIAS')

	AND tbc.tbca_tmultimaalteracao < '02/09/2021'
	--AND tbL.tbla_id1 = 1760670
	--AND imo.imov_id = 6774490
UNION
	SELECT
	imo.imov_id AS "MATRICULA",
	cli.clie_nmcliente AS "NOME",
...
FROM 
	...
	INNER JOIN atendimentopublico.ligacao_agua_situacao las ON las.last_id = imo.last_id
	INNER JOIN atendimentopublico.ligacao_esgoto_situacao les ON les.lest_id = imo.lest_id
	INNER JOIN cadastro.cliente_fone cfn ON cfn.clie_id = cli.clie_id
	INNER JOIN seguranca.tabela_linha_alteracao tbl ON /*tbl.tabe_id = 60 AND */ tbl.tbla_id1 = cfn.cfon_id
	INNER JOIN seguranca.tab_linha_col_alteracao tbc ON tbl.tbla_id = tbc.tbla_id
	INNER JOIN seguranca.tabela_coluna tco ON tco.tbco_id = tbc.tbco_id
	INNER JOIN seguranca.usuario_alteracao usa ON usa.tref_id = tbl.tref_id
	INNER JOIN seguranca.usuario usu ON usu.usur_id = usa.usis_id
