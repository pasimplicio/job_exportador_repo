...
select
	...
	lts .ltst_dsleiturasituacao AS "Situacao de leitura",	
	...
	ftm.ftsm_dsfatsitmotivo AS "Motimo",
	fsh.ftsh_dsobservacaoinforma AS "Obs faturamento",	
	TO_CHAR(fatur.valor, '999G999G990D00') AS "VALOR TOTAL"
...
from
	...
	left join faturamento.mov_conta_prefaturada mcpf on mcpf.imov_id = imov.imov_id and mcpf.mcpf_ammovimento = 202508 and mcpf.medt_id = 1
	left join micromedicao.consumo_historico mich on mich.imov_id = imov.imov_id and mich.cshi_amfaturamento = 202508 and mich.lgti_id = 1
	left join micromedicao.consumo_anormalidade csan on csan.csan_id = mich.csan_id 
	left join micromedicao.medicao_historico mdhi on mdhi.hidi_id = hidi.hidi_id and mdhi.mdhi_amleitura = 202508
	left join micromedicao.leitura_situacao lts ON lts.ltst_id = mdhi.ltst_idleiturasituacaoatual	
	...
	LEFT JOIN faturamento.fatur_situacao_hist fsh ON fsh.imov_id = imov.imov_id AND fsh.ftsh_amfaturamentoretirada IS NULL
	...
	inner join (
			select
				...
				cnta.cnta_vlagua+cnta.cnta_vlesgoto+cnta.cnta_vldebitos-cnta.cnta_vlcreditos-cnta.cnta_vlimpostos AS valor
			from
				faturamento.conta cnta
			where
				cnta.cnta_amreferenciaconta = 202508
				and cnta.dcst_idatual IN (0,1,2,5) 
			...
			select
				...
				cnhi.cnhi_vlagua+cnhi.cnhi_vlesgoto+cnhi.cnhi_vldebitos-cnhi.cnhi_vlcreditos-cnhi.cnhi_vlimpostos AS valor
			from
				faturamento.conta_historico cnhi
			where
				cnhi.cnhi_amreferenciaconta = 202508
				and cnhi.dcst_idatual IN (0,1,2,5) 				
