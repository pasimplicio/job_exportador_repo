	...
	FROM
		...
		) AS arrecadacao ON cat.catg_id = arrecadacao.catg_id AND per.iper_id = arrecadacao.iper_id
	...
	FROM
		...
		(
			...
			GROUP BY 1,2,3,4,5,6,7,8,9
		) AS devolucoes ON cat.catg_id = devolucoes.catg_id AND per.iper_id = devolucoes.iper_id)AS arr
	GROUP BY 1,2,3,4,5,6,7,8,9 
UNION 
	SELECT
		'FATURAMENTO' AS "TIPO",
	...
	FROM
		...
		GROUP BY 1,2,3,4,5,6,7) AS faturamento