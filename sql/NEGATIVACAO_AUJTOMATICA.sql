...
FROM 
	...
	LEFT JOIN (	SELECT 
			...
			WHERE 
				...
				AND con4.cnta_dtvencimentoconta >= (CURRENT_DATE - INTERVAL '5y')
...
WHERE
	...
	NOT EXISTS( SELECT  ics.cbst_id FROM cadastro.imovel_cobranca_situacao ics WHERE ics.imov_id = imo.imov_id AND ics.iscb_dtretiradacobranca IS NULL AND ics.cbst_id IN (12,14,17)) AND
	...
	cli.clie_iccpfcnpjvalidado = 1 AND
	imo.imov_idcategoriaprincipal < 4 AND
	imo.iper_id <> 6 AND
	con_atraso.valor >= 25.49 AND
	NOT EXISTS( 
	SELECT 
		ngc.ngcm_id 
	FROM 
	...
	NOT hid.hidr_nnhidrometro IS NULL AND
	(NOT fat.ftst_id IN (1,14,17) OR fat.ftst_id IS NULL)
