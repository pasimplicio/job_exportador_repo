...
SELECT 
	...
	(CASE 
	WHEN imo.imov_idcategoriaprincipal = 1 THEN '1 - RESIDENCIAL'
	WHEN imo.imov_idcategoriaprincipal  = 2 THEN '2 - COMERCIAL'
	WHEN imo.imov_idcategoriaprincipal  = 3 THEN '3 - INDUSTRIAL'
	WHEN imo.imov_idcategoriaprincipal = 4 AND imo.imov_idsubcategoriaprincipal = 5 THEN '5 - ESTADUAL'
	WHEN imo.imov_idcategoriaprincipal = 4 AND imo.imov_idsubcategoriaprincipal = 6 THEN '6 - FEDERAL'
	ELSE '4 - MUNICIPAL'
	END) AS "CATEGORIA 2",
	...
	fat_tot_6_meses.cagua AS "TOTAL VOL AG 6M",
	TO_CHAR(fat_tot_6_meses.vl_agua, '999G999G990D00') AS "TOTAL VL AG 6M",
	fat_tot_6_meses.cesg AS "TOTAL VOL ES 6M",
	TO_CHAR(fat_tot_6_meses.vl_esgoto, '999G999G990D00') AS "TOTAL VL ES 6M",
	TO_CHAR(fat_tot_6_meses.vl_debitos, '999G999G990D00') AS "TOTAL OUTROS SERVICOS 6M",
	TO_CHAR(fat_tot_6_meses.vl_creditos, '999G999G990D00') AS "TOTAL CRED 6M",
	...
	pag_referencia."VENCIMENTO" AS "VENCIMENTO FAT M-1",
	pag_referencia."VENCIMENTO ORIGINAL" AS "VENCIMENTO ORIGINAL FAT M-1",
	pag_referencia."ATRASO VENCIMENTO ORIGINAL" AS "ATRASO VENCIMENTO ORIGINAL FAT M-1",
	pag_referencia."FAIXA ATRASO" AS "FAIXA ATRASO FAT M-1",
	pag_referencia."VOL AGUA" AS "VOL AGUA FAT M-1",
	pag_referencia."VOL ESGOTO" AS "VOL ESGOTO FAT M-1",
	pag_referencia."VL AGUA" AS "VL AGUA FAT M-1",
	pag_referencia."VL ESGOTO" AS "VL ESGOTO FAT M-1",
	pag_referencia."VL DEBITOS" AS "VL DEBITOS FAT M-1",
	pag_referencia."VL CREDITOS" AS "VL CREDITOS FAT M-1",
	pag_referencia."VL IMPOSTOS" AS "VL IMPOSTOS FAT M-1",
	pag_referencia."VALOR" AS "VALOR FAT M-1",
	pag_referencia."STATUS" AS "STATUS FAT M-1"
FROM 
	cadastro.imovel imo
	...
	LEFT JOIN (
			SELECT
				fats.mat1 AS mat1,
				SUM(fats.cagua) AS cagua,
				SUM(fats.vl_agua) AS vl_agua,
				SUM(fats.cesg) AS cesg,
				SUM(fats.vl_esgoto) AS vl_esgoto,
				SUM(fats.vl_debitos) AS vl_debitos,
				SUM(fats.vl_creditos) AS vl_creditos,
				SUM(fats.vl_impostos) AS vl_impostos,
				SUM(fats.valor) AS valor
			FROM
			(
				SELECT 
					con4.imov_id AS mat1,
				...
				SELECT 
					...
					SUM(con4.cnhi_vldebitos) AS vl_debitos,
					SUM(con4.cnhi_vlcreditos) AS vl_creditos,
					SUM(con4.cnhi_vlimpostos) AS vl_impostos,
					SUM(con4.cnhi_vlagua+con4.cnhi_vlesgoto+con4.cnhi_vldebitos-con4.cnhi_vlcreditos-con4.cnhi_vlimpostos) AS valor
			...
			GROUP BY 1
		) AS fat_tot_6_meses ON fat_tot_6_meses.mat1 = imo.imov_id
	LEFT JOIN(
			SELECT
