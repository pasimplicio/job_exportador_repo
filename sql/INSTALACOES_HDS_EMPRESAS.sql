...
SELECT 
	...
	usu.usur_nmlogin AS "USUARIO QUE INSTALOU",
	...
	his.hidi_dtretiradahidrometro AS "DATA RETIRADA HD.",
...
FROM 
	...
	INNER JOIN atendimentopublico.ligacao_agua lagu ON lagu.lagu_id = imo.imov_id
	INNER JOIN micromedicao.hidrometro_inst_hist his ON lagu.hidi_id = his.hidi_id
	INNER JOIN seguranca.usuario usu ON usu.usur_id = his.usur_idinstalacao
			...
			FROM		
				(SELECT 
					...
					WHERE
						...
						AND pag.pgmt_amreferenciaarrecadacao = 202004
					GROUP BY 1
				UNION
					SELECT 
						pag.imov_id AS imov_id,
					...
					WHERE
						pag.pgst_idatual IN (0,1)
						AND pag.pghi_amreferenciaarrecadacao = 202004
					GROUP BY 1) pags
			GROUP BY 1
				...
				(SELECT 
					...
					WHERE
						...
						AND pag.pgmt_amreferenciaarrecadacao = (
								SELECT 
									CASE (CAST(SUBSTRING(CAST(202004 AS TEXT) FROM 5 FOR 6) AS INT))
										WHEN 1 THEN CAST((CAST(SUBSTRING(CAST(202004 AS TEXT) FROM 0 FOR 5) AS INT))-1 AS TEXT)||'VAR_UNIDADE'
										ELSE CAST(202004-1 AS TEXT)
									END
							)
					GROUP BY 1
				UNION
				...
				(SELECT 
					...
					WHERE
						...
						AND pag.pgmt_amreferenciaarrecadacao = (
								SELECT 
									CASE (CAST(SUBSTRING(CAST(202004 AS TEXT) FROM 5 FOR 6) AS INT))
										WHEN 1 THEN CAST((CAST(SUBSTRING(CAST(202004 AS TEXT) FROM 0 FOR 5) AS INT))-1 AS TEXT)||'11'
										WHEN 2 THEN CAST((CAST(SUBSTRING(CAST(202004 AS TEXT) FROM 0 FOR 5) AS INT))-1 AS TEXT)||'VAR_UNIDADE'
										ELSE CAST(202004-2 AS TEXT)
									END
							)
					GROUP BY 1
				UNION
...
WHERE
	...
	emp.empr_id >= 46 AND
	his.hidi_dtinstalacaohidrometro >= '2019-11-11' AND his.hidi_dtinstalacaohidrometro <= '2020-04-30'
